...
# Version: mobile-paste-safe (no triple-quoted strings).
...
# ––––– Extraction prompt (built from single-line strings) –––––

EXTRACTION_PROMPT = (
“You are an analyst at an Indian water-infrastructure company “
“reviewing an RFP/tender document. Extract the structured fields “
...
“- Convert capacities to MLD (1 MGD is approx 4.546 MLD; 1000 m3/d = 1 MLD).\n”
...
“- For summary, 3 to 4 sentences in plain English.\n\n”
“JSON schema:\n”
“{schema}\n\n”
“Document follows.\n”
“=====\n”
...
body = rfp_text[:120_000]
prompt = EXTRACTION_PROMPT.format(schema=schema_json, rfp_text=body)
...
st.set_page_config(page_title=“Water Sector Bid Extractor”, page_icon=”:droplet:”, layout=“wide”)
st.title(“Water Sector Bid Extractor”)
st.caption(“Upload tender PDFs and get a structured spreadsheet of bid metadata.”)
...
uploaded = st.file_uploader(“Drag in tender PDFs”, type=[“pdf”], accept_multiple_files=True)
...
for i, f in enumerate(uploaded):
    status.text("[" + str(i + 1) + "/" + str(len(uploaded)) + "] " + f.name)
    try:
        ...
        errors.append(f.name + ": schema/JSON error - " + str(e))
...
if errors:
    with st.expander("Failed: " + str(len(errors)) + " file(s)"):
        for err in errors:
            st.write(err)

if rows:
    df = pd.DataFrame(rows)
    st.success("Extracted " + str(len(rows)) + " tender(s).")
